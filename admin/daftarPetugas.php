...
if (!$data->foto || $data->foto != null) {
	...
	if (isset($gambar)) {
		if(file_put_contents(__DIR__ . "/petugas/$namaFile", $gambar)) {
...
$stmt = $conn->prepare("INSERT INTO petugas (user_id, divisi) VALUES (? , ?)");
$stmt->bind_param("ii", $id, $divisi);
if (!$stmt->execute()) {
	$pesan = array(
			"status"=>"gagal",
			"pesan"=>"kesalahan pada server"
	...
	if (!$auth_id || $auth_id == null) {
			$pesan = array(
				"status"=>"berhasil",
				"pesan"=>"petugas baru berhasil ditambahkan, gunakan email untuk Login",
