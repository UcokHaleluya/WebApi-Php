...
$reply = array();
$quest = array();
$stmt = $conn->prepare("SELECT a.judul, a.waktu, a.pesan, a.file, a.pengirim as user_id,CONCAT(b.nama_depan,' ',b.nama_belakang) as pengirim, b.foto FROM kiriman_konsul a INNER JOIN user b ON a.pengirim = b.id WHERE a.id = ? AND a.pengirim = ?");
$stmt->bind_param("ii", $kiriman_id, $user_id);
if (!$stmt->execute()) {
	$pesan = array(
			"status"=>"gagal",
