...
$ruangan = array();
$tambahan = array();
$stmt = $conn->prepare("SELECT dokter,diagnosa_awal,kode_diagnosa_awal,diagnosa_akhir,kode_diagnosa_akhir,operasi,penyakit,terapi, kondisi_pulang,tanggal FROM resume_medis WHERE id = ?");
$stmt->bind_param("i", $no_resume);
if (!$stmt->execute()) {
	$pesan = array(
			"status"=>"gagal",
